...
<sub>Summer 2026 · EP–SFT, Software Frameworks and Tools</sub>

The [LCG software stacks](https://gitlab.cern.ch/sft/stacks/lcgcmake) are built centrally and published to CVMFS: 900+ packages each at a fixed version, about 50 stack and platform combinations rebuilt every night. All of it runs on Jenkins today, and a build is requested from outside the repository, by a merge-request comment that a bot parses and passes on to Jenkins. Nobody had tried the builds in GitLab CI/CD. I built both halves: merge-request builds, now merged into master, and a nightly chain that compiles, publishes to CVMFS, creates the views and runs the post-publish suites unattended on a pilot stack.

CVMFS allows one writer at a time, so the publishing jobs have to wait for each other. Jenkins guards that with two mechanisms, a lock for each stack and a shared pool of slots. GitLab reproduces the lock in one line of configuration and has no equivalent for the pool, so the situation it was written for has to be handled by agreement instead of by the tool. That is the only place where the answer is not a plain yes. A night takes about 70 minutes end to end, 92% of it serialized on the single publisher, so adding build runners would not shorten it. The chain ran against a test CVMFS instance rather than production.
...
Image and video diffusion models are trained on latents that are still pixel grids: smaller than the input, but grids. A grid gives every region the same number of values, while the information concentrates around details. The project replaces the grid with a set of 2D Gaussian splats (via Image-GS), which concentrate where the information density is highest. A VAE then encodes those splat parameters into a latent vector that a generative model can be trained on, in place of the grid latents used today. One representation should serve both compression and generation. Compression needs few bits for the latent and a reconstruction close to the input. Generation adds a third requirement: the latent distribution has to be learnable by a diffusion model. Morton Z-order sorting linearises the splats first, so the 1D convolutions have local spatial structure to find.

Position and scale converge. Rotation and colour do not, and scaling the decoder up did not fix it. My hypothesis was that the decoder is the bottleneck, but the deeper flaw is running an autoencoder over an input that is fundamentally a set: mapping a latent vector back to a set of parameters with complex interdependencies stays hard even with Morton ordering. No generative model has been trained on the latents yet. Work on the project was paused in February 2026, and the underlying question moved to my thesis.
